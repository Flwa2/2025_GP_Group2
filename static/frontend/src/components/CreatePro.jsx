...
const getFilteredVoicesForSpeaker = (speakerIndex) => {
  ...
  return voices.filter((v) => {
    ...
    // Export script as PDF function
    const exportScriptAsPDF = () => {
        if (!generatedScript) {
        ...
        doc.text(`WeCast Podcast Script - ${styleLabelMap[scriptStyle] || scriptStyle} Style`, 20, 35);
        ...
        // Save the PDF
        const fileName = `${(showTitle || "podcast_script").replace(/[^a-z0-9]/gi, '_').toLowerCase()}.pdf`;
        doc.save(fileName);
    };
                    ...
                    {step === 4 && generatedScript && (
                        <section className="ui-card">
                            <div className="flex items-center justify-between mb-4">
                                <h2 className="ui-card-title flex items-center gap-2 mb-0">
